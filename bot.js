...
client.on("message" , async message => {
 ...
 if(afk){
   ...
   if(message.content.includes(kisi3)){
     const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setAuthor("Kenas" , client.user.avatarURL)
      .setDescription(`<a:kral:778691335058227200>   Etiketlediğiniz Kişi Afk \n <a:kirmiziyildiz:776160729006473247>   Sebep : ${sebep}`)
      .setTimestamp()
  ...
  if(message.author.id === kisi){
    const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setAuthor("Kenas" , client.user.avatarURL)
      .setDescription(`<a:kral:778691335058227200>   Afk'lıktan Çıktınız`)
      .setTimestamp()
       message.channel.send(embed)
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm') {
          try {
                  ...
                  return msg.reply('**<a:kirmiziyildiz:776160729006473247> Aleyküm Selam, Hoşgeldin.** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildCreate", async guild => {
 ...
 devtr.send('<a:yuklenyor:776398699596218398>  | **Beni Sunucuna Eklediğin İçin Teşekkürler |  \n\n `=>` **+yardım** yazarak komutlarıma erişebilirsiniz! \n\n `=>`Destek sunucumuz: https://discord.gg/PryH4MYH9D')
})
...
client.guilds.get('767665070154973194').members.get(id).addRole('775733215120982056')
}) //DevTR

client.on('message', msg => {
    if (msg.content === 'kenas botun sahibi kim') {
      msg.reply('**Benim sahibim:** <@692466335526944838>');
    }
});
client.on('message', msg => {
    if (msg.content === 'kenas botun sahibi kim?') {
...
client.on('message', msg => {
    if (msg.content === 'kenas botun sahibi kimdir') {
      msg.reply('**Benim sahibim:** <@692466335526944838>');
    }
});
...
client.on('guildMemberAdd', async member => {
    ...
    let kayıtcırol = "768822753649754133" //Yetkili rolünüz ID'sini girin.
    let kanal = client.channels.get("767801668942430209") //Kanalınızın ID'sini girin.
    kanal.send(`Merhaba <@${member.user.id}> hanedanımıza **hoşgeldin!**\n\nSeninle beraber **${kişi}** kişiyiz.\n\nHesap kuruluş tarihi; **${tarih}** [**${gün}** gün önce].`, resim)
})


// eklendim
client.on('guildCreate', async guild => { client.channels.get('785898963236290571').send(`${guild}, isimli sunucuya eklendim!`)})
// atıldım
client.on('guildRemove', async guild => { client.channels.get('785898963236290571').send(`${guild}, isimli sunucudan atıldım.. :(`)})