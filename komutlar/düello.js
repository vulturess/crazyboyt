...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
        ...
        if (!opponent) return message.reply("<a:hayir:769966838192275486>  **Oynamak istediğin kişiyi etiketlemelisin.**")
  
  if (opponent.bot) return message.reply('<a:hayir:769966838192275486>  **Botlar ile düello atamazsın.**');
  if (opponent.id === message.author.id) return message.reply('<a:hayir:769966838192275486>  **Kendinle düello atamazsın.**');
                ...
                await message.channel.send(`${opponent}, düello isteği geldi. Düello'yu kabul ediyor musun? (\`evet\` veya \`hayır\`) olarak cevap veriniz.)`);
                                ...
                                if (!verification) {
                                        ...
                                        return message.channel.send(`<a:hayir:769966838192275486>  **Düello kabul edilmedi.** `);
                                }
                        }
                        let userHP = 200;
                        let oppoHP = 200;
                        ...
                        while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
                                ...
                                if (!opponent.bot || (opponent.bot && userTurn)) {
                                        await message.channel.send(stripIndents`
                                                ${user}, ne yapmak istersin? \`saldır\`, \`savun\`, \`super güç\`, veya \`kaç\`?
                                ...
                                if (choice === 'saldır') {
                                        ...
                                        if (!miss) {
                                                ...
                                                dealDamage(damage);
                                        } else {await message.channel.send(`${user}, Çoook uzak galaksilerden gelen ultra sonik enerjiyi yeterli miktarda toplayamadığın için ulta güç kullanamadın!`);
                                        }
                                        reset();
                                } else if (choice === 'kaç') {
