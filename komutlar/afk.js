...
exports.run = async (client, message, args, member) => {
  ...
  const a = await db.fetch(`<a:hype:775380937528836096>  afkSebep_${message.author.id}_${message.guild.id}`)
 
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setAuthor("Kenas" , client.user.avatarURL)
      .setDescription(` <a:evet:767699993108152320> Başarıyla Afk Oldunuz \n    Sebep: ${a}`)
      .setTimestamp()
  ...
  if(args[0]){
    ...
    await db.set(`afkAd_${message.author.id}_${message.guild.id}`, b)
     ...
     const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setAuthor("Kenas" , client.user.avatarURL)
      .setDescription(`<a:tik:772430920182661120> >  Başarıyla Afk Oldunuz \n   Sebep: ${a}`)
      .setTimestamp()
       message.channel.send(embed)
